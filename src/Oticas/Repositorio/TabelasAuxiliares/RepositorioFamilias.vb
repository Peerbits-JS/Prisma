...
Namespace Repositorio.TabelasAuxiliares
    Public Class RepositorioFamilias
        Inherits RepositorioGenerico(Of Oticas.BD.Dinamica.Aplicacao, tbFamilias, Familias)
...
#Region "LEITURA"
        Protected Overrides Function ListaCamposTodos(query As IQueryable(Of tbFamilias)) As IQueryable(Of Familias)
            Return query.Select(Function(e) New Familias With {
        ...
        Protected Overrides Function ListaCamposCombo(query As IQueryable(Of tbFamilias)) As IQueryable(Of Familias)
            Return query.Select(Function(e) New Familias With {
        ...
        ' LISTA
        Public Overrides Function Lista(inFiltro As ClsF3MFiltro) As IQueryable(Of Familias)
            Return ListaCamposTodos(FiltraQuery(inFiltro))
        End Function

        ' LISTA FILTRADO
        Public Overrides Function ListaCombo(inFiltro As ClsF3MFiltro) As IQueryable(Of Familias)
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Protected Overrides Function FiltraQuery(inFiltro As ClsF3MFiltro) As IQueryable(Of tbFamilias)
            Dim query As IQueryable(Of tbFamilias) = tabela.AsNoTracking
