...
Namespace Repositorio.TabelasAuxiliares
    Public Class RepositorioTiposTratamentosLentes
        Inherits RepositorioGenerico(Of Oticas.BD.Dinamica.Aplicacao, tbTiposTratamentosLentes, TiposTratamentosLentes)
...
#Region "LEITURA"
        Protected Overrides Function ListaCamposTodos(query As IQueryable(Of tbTiposTratamentosLentes)) As IQueryable(Of TiposTratamentosLentes)
            Return query.Select(Function(entity) New TiposTratamentosLentes With {
        ...
        Protected Overrides Function ListaCamposCombo(query As IQueryable(Of tbTiposTratamentosLentes)) As IQueryable(Of TiposTratamentosLentes)
            Return query.
                Select(Function(entity) New TiposTratamentosLentes With {.ID = entity.ID, .Descricao = entity.Descricao}).
                Take(TamanhoDados.NumeroMaximo)
        End Function

        ' LISTA
        Public Overrides Function Lista(inFiltro As ClsF3MFiltro) As IQueryable(Of TiposTratamentosLentes)
            Return ListaCamposTodos(FiltraQuery(inFiltro))
        End Function

        ' LISTA FILTRADO
        Public Overrides Function ListaCombo(inFiltro As ClsF3MFiltro) As IQueryable(Of TiposTratamentosLentes)
            Return ListaCamposCombo(FiltraQuery(inFiltro))
        End Function

        Public Function ListaTiposTratamentosLentes(IDModelo As Long?) As List(Of TiposTratamentosLentes)
            'Return tabela.
        ...
        ' FILTRA LISTA
        Protected Overrides Function FiltraQuery(inFiltro As ClsF3MFiltro) As IQueryable(Of tbTiposTratamentosLentes)
            Dim query As IQueryable(Of tbTiposTratamentosLentes) = tabela.AsNoTracking()
...
#Region "FUNÇÕES AUXILIARES"
        ...
        Public Function ProximoCodigo() As String
            Return ClsUtilitarios.AtribuirCodigo(BDContexto, "tbTiposTratamentosLentes")
        End Function
#End Region
    End Class
End Namespace