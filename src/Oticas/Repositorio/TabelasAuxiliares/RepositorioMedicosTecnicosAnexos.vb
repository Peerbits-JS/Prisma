...
Namespace Repositorio.TabelasAuxiliares
    Public Class RepositorioMedicosTecnicosAnexos
        Inherits RepositorioGenerico(Of Oticas.BD.Dinamica.Aplicacao, tbMedicosTecnicosAnexos, F3M.Anexos)
...
#Region "LEITURA"
        Protected Overrides Function ListaCamposTodos(query As IQueryable(Of tbMedicosTecnicosAnexos)) As IQueryable(Of F3M.Anexos)
            Dim listaAnexos = query.Select(Function(e) New F3M.Anexos With {
                .ID = e.ID, .IDChaveEstrangeira = e.IDMedicoTecnico, .DescricaoChaveEstrangeira = e.tbMedicosTecnicos.Nome,
        ...
        Protected Overrides Function ListaCamposCombo(query As IQueryable(Of tbMedicosTecnicosAnexos)) As IQueryable(Of F3M.Anexos)
        ...
        ' FILTRA LISTA
        Protected Overrides Function FiltraQuery(inFiltro As ClsF3MFiltro) As IQueryable(Of tbMedicosTecnicosAnexos)
            Dim query As IQueryable(Of tbMedicosTecnicosAnexos) = tabela.AsNoTracking
            ...
            query = query.Where(Function(o) o.IDMedicoTecnico = IDCli)
