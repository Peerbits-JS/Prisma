...
Namespace Repositorio.Documentos
    Public Class RepositorioDocumentosComprasAnexos
        Inherits RepositorioGenerico(Of Oticas.BD.Dinamica.Aplicacao, tbDocumentosComprasAnexos, F3M.Anexos)
...
#Region "LEITURA"
        Protected Overrides Function ListaCamposTodos(query As IQueryable(Of tbDocumentosComprasAnexos)) As IQueryable(Of F3M.Anexos)
            Dim listaAnexos = query.Select(Function(e) New F3M.Anexos With {
                .ID = e.ID,
                .IDChaveEstrangeira = e.IDDocumentoCompra,
        ...
        Protected Overrides Function ListaCamposCombo(query As IQueryable(Of tbDocumentosComprasAnexos)) As IQueryable(Of F3M.Anexos)
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Protected Overrides Function FiltraQuery(inFiltro As ClsF3MFiltro) As IQueryable(Of tbDocumentosComprasAnexos)
            Dim query As IQueryable(Of tbDocumentosComprasAnexos) = tabela.AsNoTracking
            Dim IDocStk As Long = ClsUtilitarios.RetornaValorKeyDicionarioCamposFiltrar(inFiltro, CamposGenericos.IDChaveEstrangeira, GetType(Long))

            AplicaFiltroAtivo(inFiltro, query)

            query = query.Where(Function(o) o.IDDocumentoCompra = IDocStk).OrderBy(Function(o) o.ID)
