...
Namespace Repositorio.Documentos
    Public Class RepositorioDocumentosStockAnexos
        Inherits RepositorioGenerico(Of Oticas.BD.Dinamica.Aplicacao, tbDocumentosStockAnexos, F3M.Anexos)
...
#Region "LEITURA"
        Protected Overrides Function ListaCamposTodos(query As IQueryable(Of tbDocumentosStockAnexos)) As IQueryable(Of F3M.Anexos)
            Dim listaAnexos = query.Select(Function(e) New F3M.Anexos With {
                .ID = e.ID,
                .IDChaveEstrangeira = e.IDDocumentoStock,
        ...
        Protected Overrides Function ListaCamposCombo(query As IQueryable(Of tbDocumentosStockAnexos)) As IQueryable(Of F3M.Anexos)
        ...
        ''' <returns></returns>
        ''' <remarks></remarks>
        Protected Overrides Function FiltraQuery(inFiltro As ClsF3MFiltro) As IQueryable(Of tbDocumentosStockAnexos)
            Dim query As IQueryable(Of tbDocumentosStockAnexos) = tabela.AsNoTracking
            ...
            query = query.Where(Function(o) o.IDDocumentoStock = IDocStk).OrderBy(Function(o) o.ID)
