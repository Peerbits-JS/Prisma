...
Namespace Repositorio.Documentos
    Public Class RepositorioDocumentosVendasAnexos
        Inherits RepositorioGenerico(Of Oticas.BD.Dinamica.Aplicacao, tbDocumentosVendasAnexos, F3M.Anexos)
...
#Region "LEITURA"
        Protected Overrides Function ListaCamposTodos(query As IQueryable(Of tbDocumentosVendasAnexos)) As IQueryable(Of F3M.Anexos)
            Dim listaAnexos = query.Select(Function(e) New F3M.Anexos With {
                .ID = e.ID, .IDChaveEstrangeira = e.IDDocumentoVenda, .DescricaoChaveEstrangeira = e.tbDocumentosVendas.NumeroDocumento,
        ...
        Protected Overrides Function ListaCamposCombo(query As IQueryable(Of tbDocumentosVendasAnexos)) As IQueryable(Of F3M.Anexos)
        ...
        ' FILTRA LISTA
        Protected Overrides Function FiltraQuery(inFiltro As ClsF3MFiltro) As IQueryable(Of tbDocumentosVendasAnexos)
            Dim query As IQueryable(Of tbDocumentosVendasAnexos) = tabela.AsNoTracking
            ...
            query = query.Where(Function(o) o.IDDocumentoVenda = IDCli)
